function [glob, depos] = calculateStatistics (glob, depos, xco, yco)
% Calculate stratigraphic completeness and proportion of total area covered stats for each time interval in the strata
    ...
    for x = 1:glob.xSize
        ...
        for y = 1:glob.ySize
            ...
            if nEvents >= 20 % && length(nonzeros(depos.transThickness(y,x,:))) > 10
                [glob.runsAnalysisPValueMap(y,x),glob.runsOrderMetric(y,x)] = oneSectionRunsAnalysis(depos.transThickness(y,x,:),0,minimumThickness); % Zero is the flag value for no graphics plotting
    ...
    if sum(depos.transThickness(50,100,:)) > 0.01
        [dummy,~] = oneSectionRunsAnalysis(depos.transThickness(50,100,:),1,minimumThickness); % One is the flag value for graphics plotting
    end
    if sum(depos.transThickness(80,100,:)) > 0.01
        [dummy,~] = oneSectionRunsAnalysis(depos.transThickness(65,100,:),1,minimumThickness);
    end
    if sum(depos.transThickness(79,94,:)) > 0.01
        [dummy,~] = oneSectionRunsAnalysis(depos.transThickness(80,100,:),1,minimumThickness);
    end
    if sum(depos.transThickness(70,90,:)) > 0.01
        [dummy,~] = oneSectionRunsAnalysis(depos.transThickness(60,90,:),1,minimumThickness);
    end% 
