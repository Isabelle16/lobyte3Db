function [runsPValueSumAboutMean,runsOrderMetricAboutMean] = oneSectionRunsAnalysisAboutMean(thicknesses, plotGraphicsFlag, minimumThickness)
...
runsOrderMetricAboutMean = calculateRunsAboutMeanOrderMetric(thicknesses);
...
for j = 1:maxIterations
    ...
    multiRunsOrderMetricDataShuffled(j) = calculateRunsAboutMeanOrderMetric(shuffledThick);
end
...
runBinIndex = 1 + int16(((runsOrderMetricAboutMean-minRun)/runRange)*(runRange/runBinIncrement)); % Position of runs stat in the histogram
runsPValueSumAboutMean = sum(multiRunsOrderMetricDataShuffledHistData(runBinIndex:length(multiRunsOrderMetricDataShuffledHistData))); % area under curve from r to max run value
...
if plotGraphicsFlag
    plotMCRunsHistogram(minRun, runBinIncrement, maxRun, multiRunsOrderMetricDataShuffledHistData, runsOrderMetricAboutMean);
end
end

function runsOrderMetric = calculateRunsAboutMeanOrderMetric(thicknesses)
...
% Calculate the change relative to the mean thickness
meanThickness=median(thicknesses);
deltaThick=thicknesses-meanThickness;
